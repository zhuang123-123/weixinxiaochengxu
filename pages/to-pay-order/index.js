...
Page({
  ...
  data: {
    ...
    goodsJsonStr: '',
    orderType: '',  // 订单类型，购物车下单或立即支付下单，默认时购物车
    pingtuanOpenId: undefined,  // 平团的话记录团号
    ...
    allowSelfCollection: '0',  //是否允许到店自取
    ...
    WXAPI.orderCreate(postData).then(function (res) {
      ...
      if (!e) {
        that.setData({
          ...
          allGoodsPrice: res.data.amountLogistics + res.data.amountTotle,
  ...
  async doneShow() {
    ...
    let shopList = [];
    const token = wx.getStorageSync('tokem')
    // 立即购买下单
    if ('buyNow' == this.data.orderType) {
      var buyNowInfoMen = wx.getStorageSync('buyNowInfo');
      this.data.kjId = buyNowInfoMen.kjId;
      if (buyNowInfoMen && buyNowInfoMen.shopList) {
        shopList = buyNowInfoMen.shopList
      }
    } else {
      // 购物车下单
